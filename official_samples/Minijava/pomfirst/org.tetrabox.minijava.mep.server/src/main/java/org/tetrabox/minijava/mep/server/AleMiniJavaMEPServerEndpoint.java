...
@ServerEndpoint("/mep/minijava")
@ApplicationScoped
public class AleMiniJavaMEPServerEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(AleMiniJavaMEPServerEndpoint.class);
...
//	LanguageServerImpl server = new LogoIdeSetup().createInjectorAndDoEMFRegistration().getInstance(LanguageServerImpl.class);
	...
	AleMiniJavaGemocMEPServerImpl server;
	
	public AleMiniJavaGemocMEPServerImpl getOrCreateServer() {
		if(server == null) {
			server = new AleMiniJavaMEPModule().createInjectorAndDoEMFRegistration().getInstance(AleMiniJavaGemocMEPServerImpl.class);
		}
		return server;
	}
