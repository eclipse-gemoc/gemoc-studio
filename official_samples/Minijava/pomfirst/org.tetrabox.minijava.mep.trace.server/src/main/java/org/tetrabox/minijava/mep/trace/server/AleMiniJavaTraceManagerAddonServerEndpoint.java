...
@ServerEndpoint("/mep/minijava/trace")
@ApplicationScoped
public class AleMiniJavaTraceManagerAddonServerEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(AleMiniJavaTraceManagerAddonServerEndpoint.class);
	...
	public TraceManagerAddonProtocolServer getOrCreateServer() {
		if(server == null) {
			FakeOSGI.start();
			IRemoteTraceAddon traceAddon = new AleMiniJavaMEPModule().createInjectorAndDoEMFRegistration().getInstance(RemoteTraceManagerAddon.class);
