...
@ServerEndpoint("/mep/minijavarepl")
@ApplicationScoped
public class AleMiniJava_replMEPServerEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(AleMiniJava_replMEPServerEndpoint.class);
...
//	LanguageServerImpl server = new LogoIdeSetup().createInjectorAndDoEMFRegistration().getInstance(LanguageServerImpl.class);
	...
	AleMiniJava_replGemocMEPServerImpl server;
	
	public AleMiniJava_replGemocMEPServerImpl getOrCreateServer() {
		if(server == null) {
			server = new AleMiniJava_replMEPModule().createInjectorAndDoEMFRegistration().getInstance(AleMiniJava_replGemocMEPServerImpl.class);
		}
		return server;
	}
	
    void onStart(@Observes StartupEvent ev) {               
        LOGGER.info("The AleMiniJava_replMEPServerEndpoint is starting...");
		...
		//} catch (InterruptedException e) {
			...
			LOGGER.error("failed to create Xtext AleMiniJava_replMEPModule, classpath may be inconsistent");
