...
import org.eclipse.gemoc.executionframework.mep.omniscient.IOmniscientDebugProtocolClient;
import org.eclipse.gemoc.executionframework.mep.omniscient.OmniscientDebugProtocolServer;
...
@ServerEndpoint("/mep/minijavarepl/omniscient")
@ApplicationScoped
public class AleMiniJava_replOmniscientDebugServerEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(AleMiniJava_replOmniscientDebugServerEndpoint.class);
	...
	OmniscientDebugProtocolServer server;
	
	public OmniscientDebugProtocolServer getOrCreateServer() {
		if(server == null) {
			server = new OmniscientDebugProtocolServer();
		}
		return server;
	}
	
    void onStart(@Observes StartupEvent ev) {               
        LOGGER.info("The AleMiniJava_replOmniscientDebugServerEndpoint is starting...");
	...
	public void onOpen(Session session) {
		...
		try {
			...
			Launcher<IOmniscientDebugProtocolClient> serverSideLauncher = MEPLauncher.createLauncher(server, IOmniscientDebugProtocolClient.class, in, outputstream);
