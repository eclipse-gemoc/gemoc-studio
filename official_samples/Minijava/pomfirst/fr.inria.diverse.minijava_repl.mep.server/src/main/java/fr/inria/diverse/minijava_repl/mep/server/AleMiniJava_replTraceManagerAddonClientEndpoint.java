...
@ClientEndpoint
@ApplicationScoped
public class AleMiniJava_replTraceManagerAddonClientEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(AleMiniJava_replTraceManagerAddonClientEndpoint.class);

	boolean supportHeader = true;
	
	Map<Session, ExtensibleInputStream> activeSessions = new HashMap<Session, ExtensibleInputStream>();

	public volatile static AleMiniJava_replTraceManagerAddonClientEndpoint instance = null;
	
	TraceManagerAddonProtocolClient client;
	
	public AleMiniJava_replTraceManagerAddonClientEndpoint() {
		instance = this;
	}
	...
	public TraceManagerAddonProtocolClient getOrCreateClient() {
		if(client == null) {
			...
			if (engine instanceof AleReplEngineMEP<?>) {
				((AleReplEngineMEP<?>) engine).setTraceAddon(client);
			}
