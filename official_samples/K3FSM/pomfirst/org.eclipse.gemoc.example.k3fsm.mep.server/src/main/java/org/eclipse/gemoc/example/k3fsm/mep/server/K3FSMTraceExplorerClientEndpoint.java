...
@ClientEndpoint
@ApplicationScoped
public class K3FSMTraceExplorerClientEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(K3FSMTraceExplorerClientEndpoint.class);
	...
	K3FSMTraceManagerAddonClientEndpoint traceAddon;
	
	public K3FSMTraceExplorerClientEndpoint(K3FSMTraceManagerAddonClientEndpoint traceAddon) {
	...
	public TraceExplorerProtocolClient getOrCreateClient() {
		if(client == null) {
			...
			K3FSMTraceManagerAddonClientEndpoint.instance.getOrCreateClient().setTraceExplorer(client);
		}
		return client;
	}
