...
@ClientEndpoint
@ApplicationScoped
public class K3FSMTraceManagerAddonClientEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(K3FSMTraceManagerAddonClientEndpoint.class);

	boolean supportHeader = true;
	
	Map<Session, ExtensibleInputStream> activeSessions = new HashMap<Session, ExtensibleInputStream>();

	public volatile static K3FSMTraceManagerAddonClientEndpoint instance = null;
	
	TraceManagerAddonProtocolClient client;
	
	public K3FSMTraceManagerAddonClientEndpoint() {
		instance = this;
	}
	...
	public TraceManagerAddonProtocolClient getOrCreateClient() {
		if(client == null) {
			...
			if (engine instanceof PlainK3ExecutionEngineMEP<?>) {
				((PlainK3ExecutionEngineMEP<?>) engine).setTraceAddon(client);
			}
