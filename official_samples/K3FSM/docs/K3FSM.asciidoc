...
* In the _org.eclipse.gemoc.example.k3fsm project_, open the *genmodel* file, Right click on the root element then _Generate model code_; _Generate edit code_; _Generate editor code_.
...
* Edit the ecore, to add the concepts for the semantic domain. Ie. operations _run()_, _step()_, and _fire()_ and add the reference _currentState_ as presented in <<img-K3FSM_syntaxic_domain_and_semantic_domain>>.
...
* Rigth clik on the xdsml project; _GEMOC Language_ ->  _Create XText Editor Project for Language_. Add the genmodel to the list of EPackages; Name this new project "org.eclipse.gemoc.example.k3fsm.xtext"; use "org.eclipse.gemoc.example.k3fsm.K3FSM" as name for the language, and "fsm" as extensions.
   ** in the xtext editor:
