...
@ServerEndpoint("/mep/alelogo")
@ApplicationScoped
public class AleLogoMEPServerEndpoint {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(AleLogoMEPServerEndpoint.class);
...
//	LanguageServerImpl server = new LogoIdeSetup().createInjectorAndDoEMFRegistration().getInstance(LanguageServerImpl.class);
	...
	AleLogoGemocMEPServerImpl server;
	
	public AleLogoGemocMEPServerImpl getOrCreateServer() {
		if(server == null) {
			server = new AleLogoMEPModule().createInjectorAndDoEMFRegistration().getInstance(AleLogoGemocMEPServerImpl.class);
		}
		return server;
	}
