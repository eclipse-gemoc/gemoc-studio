...
include::userguide/lw_DefineModelAnimation_inSemantics.asciidoc[]
...
NOTE: if enabled, animation must occurs in both Run and Debug modes.
NOTE: TODO how to enable/disable animation
...
[[userguide-mw-debug-commands-view]]
==== Debug commands
...
ie. show line in Xtext, show current element in sirius
